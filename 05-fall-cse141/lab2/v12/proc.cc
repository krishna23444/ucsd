...
void Proc::Init() {
  ...
  // branch less than to line 15
  CMDS[0x7D].cmd_name = "blt16";
...
//    cerr << "clrr";
  ...
  else if (cmd == "blt" || cmd == "bge" || cmd == "blt16") {
    result = alu_bit_flag;
//    cerr << "B";
  }
  // bitwise and
...
//    cerr << "bge\n";
  ...
  // branch less than to line 15, check alu flag
  else if (cmd == "blt16") {
    pc = alu_bit_flag ? 16 : pc + 1;
//    cerr << "blt16\n";
  }
  else
    pc = pc + 1;
...
void Proc::GenerateInstTrace(FILE *fout, int inst_num) {
  ...
  switch (CMDS[inst_num].num_ops) {
    case 2:
      fprintf(fout, "  %s, %s", REGS[CMDS[inst_num].op1()].c_str(),
              REGS[CMDS[inst_num].op2()].c_str());
      break;
    case 1:
      if (CMDS[inst_num].is_branch)
        fprintf(fout, "  %d", CMDS[inst_num].op1());
      else
        fprintf(fout, "  %s", REGS[CMDS[inst_num].op1()].c_str());
      break;
    case 0:
    default:
      break;
