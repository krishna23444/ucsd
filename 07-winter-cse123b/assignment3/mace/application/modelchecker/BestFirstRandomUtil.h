...
// 		  //finished before the depth.
  ...
  BestFirstRandomUtil(bool v = false) :
    ...
    searched(0), depth(0),
    depthIncrement(Params::Instance()->get("SEARCH_DEPTH", 5)),
...
//     return !doneEarly && depth < maxSearchDepth;
  ...
  bool next() {
    ...
    static int mask = Params::Instance()->get("search_print_mask", 0x00000fff);
    static uint maxPaths = Params::Instance()->get("MAX_PATHS", UINT_MAX);
    if(firstTime) { 
      firstTime = false;
...
// 	 << " prefix.size=" << prefix.size() << std::endl;
  ...
  static BestFirstRandomUtil* SetInstance() { 
    ...
    if(_inst == NULL) { 
      ...
      if(Params::Instance()->containsKey("RANDOM_UTIL_PREFIX_SEARCH")) {
	std::string s = Params::Instance()->getStr("RANDOM_UTIL_PREFIX_SEARCH");
	...
	t->setPrefix(initial);
      ...
      else if(Params::Instance()->containsKey("RANDOM_UTIL_PREFIX_FILE")) {
