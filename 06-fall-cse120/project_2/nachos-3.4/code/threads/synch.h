...
class Condition {
  ...
  private:
    char* name;
    List *threadsWaitingToBeSignaledQueue;
    // plus some other stuff you'll need to define
};
...
class Mailbox {
 public:
  ...
  void Send(int newMessage);
  void Receive(int *newMessage);

 private:
  Lock *receiverLock, *senderLock;
  Semaphore *receiverSemaphore, *senderSemaphore;
  int message;
  bool hasMessage;
};
