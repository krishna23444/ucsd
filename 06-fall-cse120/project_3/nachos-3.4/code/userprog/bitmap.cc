...
{ 
  ...
  writeLock = new Lock("bitmap write lock");
...
{
  writeLock->Acquire();
  ClearNotAtomic(which);
  writeLock->Release();
}

void BitMap::ClearNotAtomic(int which)
{
  //DEBUG('b', "clearing bit %d\n", which);
  ASSERT(which >= 0 && which < numBits);
  map[which / BitsInWord] &= ~(1 << (which % BitsInWord));
}

void BitMap::MarkNotAtomic(int which)
{
  //DEBUG('b', "marking bit %d\n", which);
  ASSERT(which >= 0 && which < numBits);
  map[which / BitsInWord] |= 1 << (which % BitsInWord);
}
